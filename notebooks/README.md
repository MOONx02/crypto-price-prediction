...
- **Version = pipeline only.** The current notebook is `Crypto_Colab_AllInOne_v4.ipynb` (rolling backtest, TimeSeriesSplit + RandomizedSearchCV, residual plots).
...
- **When you add a new pipeline revision:** Move the current file to `archive/` and create `Crypto_Colab_AllInOne_v(N+1).ipynb`; update the first cell.
...
| **v4** | v3 + **rolling backtest**, **TimeSeriesSplit + RandomizedSearchCV**, **residual plots**. Current. Asset chosen via `ASSET` in notebook. |
...
- **Crypto_Colab_AllInOne_v1.ipynb**, **v2.ipynb**, **v3.ipynb** — superseded by v4.
